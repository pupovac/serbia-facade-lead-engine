...
- **Daibau.rs** is the best-known facade contractor directory in Serbia, facade-named, city-segmented, top of every Serbian search result. Its contractor phone numbers are served by `/autosecretary/getPhoneNumber/` — which is the **first line** of its `robots.txt` Disallow list. Fetching the profile page for a real contractor (`/izvodjac/rading_doo_beograd`) returns exactly one phone number: `+381 11 418 2607`, Daibau's own switchboard. Lead routing is their business model; the gate is deliberate. **Re-measured end to end on 2026-08-21 (FUZZ-39) and confirmed:** 108 `tel:` links across 59 listing pages resolve to just two distinct numbers, both Daibau's own, rendered in the site-wide footer popup on every page whatever it lists; profile pages truncate the contractor's number to a 7-digit prefix (`0603460...klik`), 2–3 digits short of a usable Serbian mobile. The record count here (75) was too low — a walk to exhaustion found **748** in `fasade` alone — and the pagination and `requires_js` readings are corrected in `sources-contractors.json`. None of it changes the verdict. Full write-up: `research/2026-08-21-fuzz-39-daibau-rs.md`.
...
- ~~**NadjiMajstora.rs** has a clean, crawlable, facade-named category with 56 registered fasaderi and a permissive `robots.txt`. I fetched five profile pages. Zero phone numbers on any of them — contact runs through an on-site form.~~ **Overturned by FUZZ-40 (2026-08-21): phone coverage is 100%, not 0%, and this is now a shipped adapter.** The profile page really does carry no number, so the sampling was accurate — but the conclusion drawn from it was not. The contact tab’s reveal button calls `POST /master/show_tel/` with `id=<masterId>` and no cookie, token or session, and that returns the number. A full crawl of `fasader` + `izolater` returned 89 profiles and 89 phone numbers. There is no contact form.

  The lesson generalises past this one row: **"no phone in the profile HTML" and "no phone" are different findings**, and only the first one was measured. Where a page renders a reveal button, the cost of settling the question is one request.
