...
| Matching and merging           | `src/lib/dedup`     | `scoreMatch` is pure and returns `merge` / `review` / `distinct` with its signals. `ingestLead` is the entry point an adapter writes through; `dedupeDatabase` sweeps. Every merge is reversible from `merge_log`; a value shared across too many businesses is quarantined and stops deciding anything. |
