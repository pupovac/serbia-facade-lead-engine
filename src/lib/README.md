...
| `normalize/` | Turning a validated raw record into the canonical field shapes: names, addresses, cities, emails, website URLs. `name.ts` produces the display name, the matching keys and the similarity score; `address.ts` does the same for a street address, folding the punctuation two sources never agree on without ever folding two house numbers together; `city.ts` resolves free text to a `data/serbia-geo.json` slug pair, or to `null` with a reason. |
...
| `classify/`  | `FACADE_CONTRACTOR` / `CONSTRUCTION_MATERIAL_STORE` / `BOTH` / `UNCLASSIFIED` / `OUT_OF_SCOPE` — the last two split what `UNKNOWN` used to hide: nothing found, versus another trade positively identified.                                                                                                                                                                                                                                           |
| `score/`     | Two scores: `relevance` (is this a lead for us?) and `contactability` (how much contact data do we hold?), plus the derived `lead_score` the export sorts on. Neither is purchase likelihood.                                                                                                                                                                                                                                                         |
